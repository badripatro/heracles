...
class Global_SSM(nn.Module):
    def __init__(self, dim):
        ...
        if dim == 64:
            self.h = 56 #H
            self.w = 56 # W
            self.local_ssm =Local_SSM(dim // 2)
            self.weight = nn.Parameter(torch.randn(dim //2, self.h, self.w, dtype=torch.float32) * 0.02)
        if dim ==128:
            self.h = 28 #H
            self.w = 28 # W
            self.local_ssm =Local_SSM(dim // 2)
        ...
        if dim == 192: #96 for large model, 64 for small and base model
            self.h = 56 #H
            self.w = 56 #W           
            self.local_ssm =Local_SSM(dim // 2)
            self.weight = nn.Parameter(torch.randn(dim //2, self.h, self.w, dtype=torch.float32) * 0.02)
    ...
    def forward(self, x, H, W):
        ...
        B, C, H, W = x.shape 
        # print('wno B, C, H, W',B, C, H, W)

        x1, x2 = torch.chunk(x, 2, dim=1)
        x1 = self.local_ssm(x1)
        ...
        x2 = dct.dct_2d(x2, (2, 3))
        x2 = x2 * self.weight
        x2 = dct.idct_2d(x2, (2, 3))
        ...
        **kwargs
        ...
        for i in range(num_stages):
            ...
            block = nn.ModuleList([Block(
                ...
                block_type='scatter' if i < 2 else 'std_att')
...
@register_model
def sct_s(pretrained=False, **kwargs):
    model = Heracles(
        stem_hidden_dim = 32,
        embed_dims = [64, 128, 320, 448], 
        num_heads = [2, 4, 10, 14], 
...
@register_model
def sct_b(pretrained=False, **kwargs):
    model = Heracles(
        stem_hidden_dim = 64,
...
@register_model
def sct_l(pretrained=False, **kwargs):
    model = Heracles(
        stem_hidden_dim = 64,
